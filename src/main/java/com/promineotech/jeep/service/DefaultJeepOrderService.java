...
private JeepOrderDao jeepOrderDao;
  ...
  public Order createOrder(OrderRequest orderRequest) {
   ...
   Color color = jeepOrderDao.fetchColor(orderRequest.getColor()).orElseThrow(() -> new NoSuchElementException("Color with ID+"
       + orderRequest.getColor() + " was not found"));
   
   Engine engine = jeepOrderDao.fetchEngine(orderRequest.getEngine()).orElseThrow(() -> new NoSuchElementException(
       "Engine with ID=" + orderRequest.getEngine() + " was not found"));
   
   Tire tire = jeepOrderDao.fetchTire(orderRequest.getTire()).orElseThrow(() -> new NoSuchElementException(
   ...
   * @return
   */
  protected List<Option> getOption(OrderRequest orderRequest) {
